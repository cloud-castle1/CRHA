...
class Trainer:
    ...
    def train_single_epoch(self, index):
        ...
        """
        if index <= 15:
            self.criterions = [WBCEWithLogitLoss() for _ in range(self.vocab.n_level())]
        else:
            self.criterions = [FocalLoss() for _ in range(self.vocab.n_level())]
        ...
        batch_id = 0
        #text_batch":词向量（8,文本长度） label_batch：标签向量（8，8929） length_batch 单一样本文本长度
    ...
    def train(self,
        ...
        for e in range(self.start_epoch + 1, n_epoch + 1):
            ...
            train_scores = self.train_single_epoch(e)#训练
...
class FocalLoss(nn.Module):
    def __init__(self, alpha, gamma=2, logits=True, reduce=True):
