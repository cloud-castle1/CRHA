...
def _train_model(train_data, valid_data, test_data,
    ...
    model = args.model(vocab, args)#选择模型
    ...
    if args.multilabel == 1:
        #criterions = [WBCEWithLogitLoss() for _ in range(vocab.n_level())]
        criterions = [FocalLoss() for _ in range(vocab.n_level())]
        #criterions = [nn.BCEWithLogitsLoss() for _ in range(vocab.n_level())]
    elif args.multilabel == 0:
    ...
    best_model, scores = trainer.train(n_epoch=args.n_epoch, patience=args.patience)#训练
...
def load_cached_data(file_path: str) -> tuple:
    ...
    with open(file_path, 'rb') as f:
        data = pickle.load(f)
        f.close()
    return data["vocab"], data["training_data"], data["valid_data"], data["test_data"]#数据标签 训练原始文本 验证原始文本 测试原始文本
...
def _create_log_file_folder(args):
    strtime = datetime.datetime.now().strftime("%d-%m-%Y-%H-%M-%S")#18-05-2022-10-48-00 获得当前时间
